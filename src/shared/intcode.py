class IntCode:
    ...
    def run(self):
        while not self.finished:
            ...
            mode3 = int(opcodeString[opcodeLastIndex - 4]) if len(opcodeString) > 4 else 0

            step = 0
            if opcode == 1:
                param1 = self.intList[self.__getMemoryLocation(1, mode1)]
                param2 = self.intList[self.__getMemoryLocation(2, mode2)]
                self.intList[self.__getMemoryLocation(3, mode3)] = param1 + param2
                step += 4
            elif opcode == 2:
                param1 = self.intList[self.__getMemoryLocation(1, mode1)]
                param2 = self.intList[self.__getMemoryLocation(2, mode2)]
                ...
                if self.__inputPointer < len(self.inputs):
                    ...
                    self.intList[self.__getMemoryLocation(1, mode1)] = int(inputVal)
                ...
                param1 = self.intList[self.__getMemoryLocation(1, mode1)]
                param2 = self.intList[self.__getMemoryLocation(2, mode2)]
                self.intList[self.__getMemoryLocation(3, mode3)] = 1 if param1 < param2 else 0
                step += 4
            elif opcode == 8:
                param1 = self.intList[self.__getMemoryLocation(1, mode1)]
                param2 = self.intList[self.__getMemoryLocation(2, mode2)]
